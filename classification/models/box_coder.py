...
from cuml.cluster import DBSCAN  # GPU 加速的 DBSCAN
import matplotlib.pyplot as plt
import numpy as np
import os

GLOBAL_PATCH_ORDER = None
...
class pointwhCoder(pointCoder):
    ...
    def forward(self, boxes, img_size, output_size):
        ...
        self.boxes = self.decode(boxes)  # 解码生成中心点和边界框
    ...
    def decode(self, rel_codes):
        ...
        # 计算 dx + dy + dh + dw 的和
        sum_values = dx + dy + dh + dw

        # 根据 sum_values 的值生成排序索引
        GLOBAL_PATCH_ORDER = torch.argsort(sum_values, dim=1, descending=True)
        #print(GLOBAL_PATCH_ORDER)
        pred_boxes = torch.zeros_like(rel_codes)
...
class test:
    ...
    def getGlobalOrder(self):
        return GLOBAL_PATCH_ORDER